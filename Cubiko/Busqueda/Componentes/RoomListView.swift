...
struct RoomListView: View {
    ...
    // TODO Update this logic according to database
    var salasFiltradasPorPiso: [SalaDisponible] {
        guard let ubicacion = pisoSeleccionado else { return salas }
        return salas.filter { $0.ubicacion == String(ubicacion) }
    }
        ...
        VStack(spacing: 0) {
            ...
            ScrollView {
                LazyVStack(spacing: 0) {
                    ForEach(salasFiltradasPorPiso, id: \.numero) { sala in
                        RoomRowView(
                            sala: sala,
                            isSelected: selectedSala?.numero == sala.numero,
                            onTap: { selectedSala = sala }
                        )
                        Divider()
                            .padding(.leading, 56)
                    }
